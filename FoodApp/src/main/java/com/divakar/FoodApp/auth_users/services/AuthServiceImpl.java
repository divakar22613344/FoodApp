...
import com.divakar.FoodApp.security.JwtUtils;
...
public class AuthServiceImpl implements AuthService {
    ...
    public Response<?> register(RegistrationRequest registrationRequest) throws BadRequestException {
        ...
        if (registrationRequest.getRoles() != null && !registrationRequest.getRoles().isEmpty()) {
            ...
            // if no roles found, get the default role
            Role defautRole = roleRepository.findByName("CUSTOMER")
                    .orElseThrow(() -> new NotFoundException("Default CUSTOMER role not found"));
            userRoles = List.of(defautRole);
        }

        User userToSave = User.builder().name(registrationRequest.getName())
    ...
    public Response<LoginResponse> login(LoginRequest loginRequest) throws BadRequestException {
        ...
        return Response.<LoginResponse>builder().statusCode(HttpStatus.OK.value()).message("Login Successful").data(loginResponse)
                .build();
    }
